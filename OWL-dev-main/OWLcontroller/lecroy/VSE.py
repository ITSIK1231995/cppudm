    ...
    def OnStatusReport(self, subsystem, state, percent_done):
        try:
            print("PEEvent::OnStatusReport - subsystem:{0}, state:{1}, progress:{2}".format(subsystem, state,
                                                                                            percent_done))
            self.__class__.controller.updateRunTimeStateInTerminal(self.__class__.hostPc, self.__class__.testLog,
                                                                   "PEEvent::OnStatusReport - subsystem:{0}, state:{1}, progress:{2}".format(subsystem, state,
                                                                                            percent_done))
        except Exception as e:
...
class VSEventHandler(object):
    def OnVScriptReportUpdated(self, new_line, tag):
        try:
            ...
            self.__class__.controller.updateRunTimeStateInTerminal(self.__class__.hostPc, self.__class__.testLog,
                                                                   "VSEventHandler::OnVScriptReportUpdated - %s tag:%s" % (new_line, tag))
        except Exception as e:
            print("VSEventHandler::OnVScriptReportUpdated - failed with exception: %s" % e)
            self.__class__.controller.updateRunTimeStateInTerminal(self.__class__.hostPc, self.__class__.testLog,
                                                                   "VSEventHandler::OnVScriptReportUpdated - failed with exception: %s" % e)
    def OnVScriptFinished(self, script_name, result, tag):
        try:
            print("VSEventHandler::OnVScriptFinished - {script_name}, {result}, {tag}".format(script_name=script_name,
                                                                                              ...
                                                                                              tag=tag))
            self.__class__.controller.updateRunTimeStateInTerminal(self.__class__.hostPc, self.__class__.testLog,
                                                                   "VSEventHandler::OnVScriptFinished - {script_name}, {result}, {tag}".format(script_name=script_name,
                                                                                              result=result,
                                                                                              tag=tag))
        except Exception as e:
            print("VSEventHandler::OnVScriptFinished - failed with exception: %s" % e)
            self.__class__.controller.updateRunTimeStateInTerminal(self.__class__.hostPc, self.__class__.testLog,
                                                                   "VSEventHandler::OnVScriptFinished - failed with exception: %s" % e)
    ...
    def OnNotifyClient(self, eventId, eventBody, tag):
        try:
            ...
            self.__class__.controller.updateRunTimeStateInTerminal(self.__class__.hostPc, self.__class__.testLog,
                                                                   "VSEventHandler::OnNotifyClient - eventId: %s, eventBody: %s, tag: %s" % (eventId, eventBody, tag))
        except Exception as e:
            print("VSEventHandler::OnNotifyClient - failed with exception: %s" % e)
            self.__class__.controller.updateRunTimeStateInTerminal(self.__class__.hostPc, self.__class__.testLog,
                                                                   "VSEventHandler::OnNotifyClient - failed with exception: %s" % e)
