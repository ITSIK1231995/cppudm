...
class VSEventHandler(object):
    ...
    def OnVScriptReportUpdated(self, new_line, tag):
        try:
            ...
            self.controller.updateTerminalAndLog(self.hostPc, self.testLog, "VSEventHandler::OnVScriptReportUpdated - %s tag:%s" % (new_line, tag))
        except Exception as e:
            print("VSEventHandler::OnVScriptReportUpdated - failed with exception: %s" % e)
            self.controller.updateTerminalAndLog(self.hostPc, self.testLog, "VSEventHandler::OnVScriptReportUpdated - failed with exception: %s" % e)
    ...
    def OnVScriptFinished(self, script_name, result, tag):
        try:
            ...
            self.controller.updateTerminalAndLog(self.hostPc, self.testLog, "VSEventHandler::OnVScriptFinished - {script_name}, {result}, {tag}".format(script_name=script_name, result=result, tag=tag))
        except Exception as e:
            print("VSEventHandler::OnVScriptFinished - failed with exception: %s" % e)
            self.controller.updateTerminalAndLog(self.hostPc, self.testLog, "VSEventHandler::OnVScriptFinished - failed with exception: %s" % e)
    ...
    def OnNotifyClient(self, eventId, eventBody, tag):
        try:
            ...
            self.controller.updateTerminalAndLog(self.hostPc, self.testLog, "VSEventHandler::OnNotifyClient - eventId: %s, eventBody: %s, tag: %s" % (eventId, eventBody, tag))
        except Exception as e:
            print("VSEventHandler::OnNotifyClient - failed with exception: %s" % e)
            self.controller.updateTerminalAndLog(self.hostPc, self.testLog, "VSEventHandler::OnNotifyClient - failed with exception: %s" % e)
...
class verificationScriptEngine():
    def startVerificationScript(self, traceFullPathAndName, vScriptFullPathAndName, hostPc, testLog, controller):
        ...
        controller.updateTerminalAndLog(hostPc, testLog, "\nVerification Script Engine procedure has started")
