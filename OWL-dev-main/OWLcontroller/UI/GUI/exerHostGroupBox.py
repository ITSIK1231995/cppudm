...
class exerHostGroupBox(QtWidgets.QGroupBox):
    def __init__(self, centralwidget, mainWindowRef):
        ...
        if self.controller.configs:
            ...
            if len(self.hosts) != 0:
                self.currHost = self.getFirstHost()
    ...
    def setColortoCurrHostCheckBox(self, ):
        self.setDefaultColorToChoosenHostPc(self.currHost)

    def addHostBtnSetup(self):
        self.addHostPcBtn = QtWidgets.QPushButton(self)
        self.addHostPcBtn.setGeometry(QtCore.QRect(130, 5, 125, 23))
    ...
    def addHostPcRow(self, hostPc):
        ...
        hostPcRowNamedtuple = namedtuple('hostPcRow',
                                         ['checkBox', 'showButton', 'editButton', 'delButton', 'containingGroupBox'])
    ...
    def getHostPCFromBtnName(self, btn):
        ...
        return next((hostPc for hostPc in self.hosts if hostPc['IP'] == hostPcIp), None)
        ...
        if self.userIsSureHeWantsTODel(hostPc["IP"]):
            ...
            if self.mainWindowRef.currentHostPc["IP"] == oldHostPCIP:
                if len(self.hosts) == 0:
                    ...
                    self.mainWindowRef.setNewHostPC(self.hosts[0])
    ...
    def setColorForPickedShowBtn(self, pickedHostPc):  # When clicking on the Show button.
        ...
        if len(self.hosts) == 1:
            self.currHost = pickedHostPc
            self.setColortoCurrHostCheckBox()
        if self.currHost["IP"] in self.controller.runtimeHostPcsData.keys() \
                and 'hostPcLblColor' in self.controller.runtimeHostPcsData[self.currHost["IP"]].keys():
            self.hostPcRows[self.currHost["IP"]].checkBox.setStyleSheet(COLOR_CONVERTER[self.controller.runtimeHostPcsData[self.currHost["IP"]]['hostPcLblColor']])
        elif self.currHost in self.hosts:
            self.hostPcRows[self.currHost["IP"]].checkBox.setStyleSheet('background-color: None')
